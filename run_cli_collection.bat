@echo off
setlocal enabledelayedexpansion

REM ============================================================
REM run_cli_collection.bat
REM   Task Scheduler wrapper for the Phoenix NWS CLI collector.
REM
REM   Responsibilities (the collector itself is unchanged):
REM     * run the collector as a module from the repo root
REM     * append all output to a dated automation log
REM     * retry ONCE after a short delay if the collector fails
REM     * exit NON-ZERO if both attempts fail (never silently succeed)
REM
REM   The collector is idempotent by product_id, so any of the three
REM   daily runs that sees an already-stored product exits cleanly
REM   with "already stored -- skipped" and writes no duplicate row.
REM
REM   Nothing here overwrites data: the collector only ever appends,
REM   and this wrapper only ever appends to the log (>>).
REM ============================================================

REM --- Fixed locations (edit REPO if you move the project) ----
set "REPO=C:\Projects\weather-pipeline"
set "DB=%REPO%\data\pipeline.db"
set "LOGDIR=%REPO%\logs"
set "PYTHON=%REPO%\venv\Scripts\python.exe"

REM --- Ensure the log directory exists ------------------------
if not exist "%LOGDIR%" mkdir "%LOGDIR%"

REM --- Build a dated log file name: automation_YYYY-MM-DD.log --
REM   WMIC gives a locale-independent timestamp (DATE format varies
REM   by machine; WMIC does not). Falls back to %DATE% if WMIC is
REM   unavailable on very new builds.
set "STAMP="
for /f "skip=1 tokens=1" %%s in ('wmic os get localdatetime 2^>nul') do if not defined STAMP set "STAMP=%%s"
if defined STAMP (
    set "TODAY=!STAMP:~0,4!-!STAMP:~4,2!-!STAMP:~6,2!"
) else (
    set "TODAY=%DATE%"
)
set "LOGFILE=%LOGDIR%\automation_!TODAY!.log"

REM --- Human-readable run timestamp for the log ---------------
set "NOW=%DATE% %TIME%"

echo. >> "%LOGFILE%"
echo ============================================================ >> "%LOGFILE%"
echo [!NOW!] CLI collection run starting (attempt 1) >> "%LOGFILE%"

REM --- Move to the repo root so relative imports resolve ------
cd /d "%REPO%"
if errorlevel 1 (
    echo [!NOW!] FATAL: cannot cd to %REPO% >> "%LOGFILE%"
    exit /b 20
)

REM --- Attempt 1 ---------------------------------------------
"%PYTHON%" -m collectors.nws_cli_collector "%DB%" >> "%LOGFILE%" 2>&1
set "RC=!errorlevel!"

if "!RC!"=="0" (
    echo [%DATE% %TIME%] SUCCESS on attempt 1 ^(exit 0^) >> "%LOGFILE%"
    exit /b 0
)

REM --- Attempt 1 failed: log, wait, retry once ---------------
echo [%DATE% %TIME%] FAILURE on attempt 1 ^(exit !RC!^) -- retrying in 60s >> "%LOGFILE%"

REM Sleep ~60s without extra dependencies. timeout needs a console;
REM /nobreak + redirect keeps it quiet under Task Scheduler.
timeout /t 60 /nobreak >nul 2>&1

echo [%DATE% %TIME%] CLI collection run starting (attempt 2) >> "%LOGFILE%"
"%PYTHON%" -m collectors.nws_cli_collector "%DB%" >> "%LOGFILE%" 2>&1
set "RC=!errorlevel!"

if "!RC!"=="0" (
    echo [%DATE% %TIME%] SUCCESS on attempt 2 ^(exit 0^) >> "%LOGFILE%"
    exit /b 0
)

echo [%DATE% %TIME%] FAILURE on attempt 2 ^(exit !RC!^) -- giving up >> "%LOGFILE%"
REM Propagate a non-zero code so Task Scheduler records "Last Run
REM Result" as a failure (0x1 etc.), never a false success.
exit /b !RC!
