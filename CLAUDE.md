...
- `archive/schema.sql` — **retired 2026-08-06 under §16; creates nothing.** Moved out of `storage/` precisely so no reader mistakes it for live DDL. It defines `collection_runs`, `nws_forecast_snapshots`, `nws_observations`, `kalshi_markets`, `kalshi_candlesticks` and `kalshi_settlements`, and **none of those tables exists in `data/pipeline.db`** — the live database holds exactly `kalshi_observations`, `raw_nws_cli`, `snapshot_blob`, `snapshot_index`, `sqlite_sequence` (Architect query, 2026-08-05). Its `PRAGMA journal_mode = WAL` has never executed; WAL is set at runtime by `storage/snapshots.py::SnapshotStore._connect()`, and any doc citing "schema.sql line 1" as the cause of WAL mode is asserting something false. `storage/schema.py` is the sole schema authority — do not add tables to the archived file.
